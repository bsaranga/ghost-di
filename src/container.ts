...
export class Container {
    ...
    deps.map((target) => {
      ...
      const childrenDep = paramTypes.map((paramType: any) => {
        ...
        if (!this.dependencies[paramType.name]) {
          (this.dependencies as any[])[paramType.name] = new paramType();
      ...
      if (!this.dependencies[target.name]) {
        (this.dependencies as any[])[target.name] = new target(...childrenDep);
      }
    });
  ...
  public get<T extends new (...args: any[]) => any>(serviceClass: T,): InstanceType<T> {
